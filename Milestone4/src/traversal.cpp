...
void Traversal::gen() {
    ...
    gen_lines.push_back("                     jal L0");
    gen_lines.push_back("                     li $v0, 10");
    gen_lines.push_back("                     syscall");

    // generate code for predefined identifiers
    
    // generate global variable declarations
    genPreOrder(root, genGlobVarDecl_cb);
    // generate remainder of code
    genPreOrder(root, gen2_cb);
...
bool Traversal::genGlobVarDecl_cb(ASTNode* node) {
    std::string label;
    // generate global variable labels
    if (node->type == "globVarDecl") {

        gen_lines.push_back("        .data");
        label = "G" + std::to_string(global_labelno++);
        node->children[1]->symtab_entry->label = label;
...
bool Traversal::freeRegisters_cb(ASTNode* node) {
    if (node->type == "formal") {
        registers.free_a_reg(node->children[1]->symtab_entry->reg);
        node->children[1]->symtab_entry->reg = "";
        return true;
    }
    return false;
}

// bool Traversal::genLoad_cb(ASTNode* node) {
//     std::string r;
//     if (node->type == "false") {
...
bool Traversal::genBlock_cb(ASTNode* node) {
    std::string r;
    if (node->type == "funcCall") {

        ASTNode* formals = node->children[1];

        std::vector<std::string> loaded_registers;
        for (size_t i = 0; i < formals->children.size(); i++) {
            r = registers.allocreg();
            loaded_registers.push_back(r);
            if (formals->children[i]->type == "false") {
                gen_lines.push_back("        li $" + r + ", 0");
            }
            if (formals->children[i]->type == "true") {
                gen_lines.push_back("        li $" + r + ", 1");
            }
            if (formals->children[i]->type == "id") {
                gen_lines.push_back("        lw $" + r + ", " + std::to_string(formals->children[i]->symtab_entry->stack_address) + "($sp)");
            }
        }
        for (size_t i = 0; i < formals->children.size(); i++) {
        ...
        gen_lines.push_back("        jal $" + node->children[0]->symtab_entry->label);
        return true;
    }
    ...
    if (node->type == "=") {
        ...
        // todo: probably refactor this for more complex expressions
        r = registers.allocreg();
        if (node->children[1]->sig == "boolean") {
        ...
        // TODO: check if storing word to a stack address, or a global label
        if (node->children[0]->symtab_entry->label != "") {
            if (node->children[0]->symtab_entry->label[0] == 'G' ) {
                gen_lines.push_back("        sw $" + r + ", " + node->children[0]->symtab_entry->label);
            }
        }
...
bool Traversal::gen2_cb(ASTNode* node) {
    std::string label;
    std::string instruction;
    std::string r;

    if (node->type == "formal") {
        r = registers.alloc_a_reg();
        node->children[1]->symtab_entry->reg = r;
        gen_lines.push_back("        sw $" + r + ", " + std::to_string(node->children[1]->symtab_entry->stack_address) + "($sp)");
    }
    if (node->type == "mainDecl" || node->type == "funcDecl") {

        // Function Header Label
        if (node->type == "mainDecl") label = "L0";
        if (node->type == "funcDecl") label = "L" + std::to_string(labelno++);
        node->children[1]->symtab_entry->label = label;
        gen_lines.push_back(label + ":");
        ...
        // Function args
        genPreOrder(node->children[2], gen2_cb);
        // Function body
        genPreOrder(node->children[3], genBlock_cb);

        // Function Footer Label
        label = "L" + std::to_string(labelno++); 
        gen_lines.push_back(label + ":");
        ...
        genPreOrder(node->children[2], freeRegisters_cb);
        stack_frame_size = 0;
